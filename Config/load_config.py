...
def load_configuration():
    ...
    env_file = r'Config\.env'
    config_file = r'Config\cam.cfg'

    # Load Credentials
    if not os.path.exists(env_file):
        logging.warning(f"Environment file '{env_file}' not found. Trying environment variables.")
    load_dotenv()
    ...
    if not config_data['cam_user'] or not config_data['cam_password']:
        logging.error(f"CAM_USER or CAM_PASSWORD not found in '{env_file}' or environment variables.")
        return None
    ...
    try:
        ...
        filter_devices_raw = config.get('Scanning', 'filter_devices', fallback='')
        ...
        if filter_devices_block_cleaned:
            ...
            logging.warning(f"'filter_devices' not found or empty in [{config_file} -> Scanning]. Scanning might include unwanted devices.")
